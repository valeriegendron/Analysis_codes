...
# Function
def sfr_profile(galaxy_name):
    # Lists containing dump number and corresponding time
    dumps, time = np.loadtxt("/home/humar50/RING/RUNS/" + str(galaxy_name) + "/DUMPS/" + str(galaxy_name) +"/tzstep.dat",
                             skiprows=1, usecols=(0, 1), unpack=True)
    str_dumps = []  # list containing dump numbers in string format
    ...
    for index in range(1, len(dumps)):  # skipping the first dump (no stars)
        ...
        for fileName in os.listdir("/home/humar50/RING/RUNS/" + str(galaxy_name) + "/DUMPS/" + str(galaxy_name)):
            if fileName.startswith("s00"):
                if fileName.endswith(str_dumps[index]):
                    smass = 0
                    smass_list = np.loadtxt("/home/humar50/RING/RUNS/" + str(galaxy_name) + "/DUMPS/" + str(galaxy_name)
                                            + "/" + fileName, usecols=6, unpack=True)
...
# Parameters
galaxy_names = ['Af_v1', 'Af_v2', 'Af_v3']
...
for name in galaxy_names:
    time, sfr = sfr_profile(name)
    ax.plot(time, sfr, label='Run ' + name)
    picName += (name + "-")
ax.set_xlabel("Time [Gyr]")
ax.set_ylabel("SFR [M$_\odot$/yr]")
ax.set_title("SFR for whole system")
plt.legend()
fig.tight_layout()

# Saving
plt.savefig(picName + "sfr_system.png", dpi=500)
