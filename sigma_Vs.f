...
*the velocity dispersion of star particles. It does so in 
...
*arrays for each bin      
     ...
     +           sigmaV(nbin,4)
      ...
      open(unit=11,file='sigma_vsc.out',status='unknown')
...
* Open the star file of our dump, read position, velocity, mass
* particles. ABCD are placeholders to some day merge different codes
*together. Luckly GCD+ already handles vr, v theta and vz
        open(unit=1,file=filename,status='old')
        ng=0
        do i=1,nmax
             read(1,*,end=99) x, y, z, vx, vy, vz, m, (dummy,j=1,13), 
     +                       flagflg, (dummy,j=1,1), A, (dummy,j=1,2), 
     +                       B, D, vph, vr 
