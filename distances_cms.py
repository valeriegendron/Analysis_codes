...
# Initialisation
# Target galaxy
tx_cm_gas, tx_cm_stars, tx_cm_dm = [], [], []
ty_cm_gas, ty_cm_stars, ty_cm_dm = [], [], []
tz_cm_gas, tz_cm_stars, tz_cm_dm = [], [], []

# Impactor galaxy
ix_cm_gas, ix_cm_stars, ix_cm_dm = [], [], []
iy_cm_gas, iy_cm_stars, iy_cm_dm = [], [], []
iz_cm_gas, iz_cm_stars, iz_cm_dm = [], [], []

tgmass, tsmass, tdmass = [], [], []  # contiendra les masses totales de particules de gas, d'etoiles et de matiere sombre
                                     # a chaque temps
igmass, ismass, idmass = [], [], []
...
for fileName in os.listdir(str(galaxy_name)):
    if fileName.endswith("_1"):  # Target galaxy
        if fileName.startswith("d00"):
            ...
            tx_cm_dm.append(x_cm*100)  # pour etre en kpc
            ty_cm_dm.append(y_cm*100)
            tz_cm_dm.append(z_cm*100)

            tdmass.append(m_tot)  # en unites de 10^12 M_sun.
        ...
        if fileName.startswith("d00"):
            ...
            ix_cm_dm.append(x_cm*100)  # pour etre en kpc
            iy_cm_dm.append(y_cm*100)
            iz_cm_dm.append(z_cm*100)

            idmass.append(m_tot)  # en unites de 10^12 M_sun.
...
for t in range(0, len(tgmass)):
    # Position
    xcm = (tgmass[t]*tx_cm_gas[t]+tsmass[t]*tx_cm_stars[t]+tdmass[t]*tx_cm_dm[t])/(tgmass[t]+tsmass[t]+tdmass[t])
    ycm = (tgmass[t]*ty_cm_gas[t]+tsmass[t]*ty_cm_stars[t]+tdmass[t]*ty_cm_dm[t])/(tgmass[t]+tsmass[t]+tdmass[t])
    zcm = (tgmass[t]*tz_cm_gas[t]+tsmass[t]*tz_cm_stars[t]+tdmass[t]*tz_cm_dm[t])/(tgmass[t]+tsmass[t]+tdmass[t])
...
for t in range(0, len(igmass)):
    # Position
    xcm = (igmass[t]*ix_cm_gas[t]+ismass[t]*ix_cm_stars[t]+idmass[t]*ix_cm_dm[t])/(igmass[t]+ismass[t]+idmass[t])
    ycm = (igmass[t]*iy_cm_gas[t]+ismass[t]*iy_cm_stars[t]+idmass[t]*iy_cm_dm[t])/(igmass[t]+ismass[t]+idmass[t])
    zcm = (igmass[t]*iz_cm_gas[t]+ismass[t]*iz_cm_stars[t]+idmass[t]*iz_cm_dm[t])/(igmass[t]+ismass[t]+idmass[t])
...
fig.tight_layout()
plt.savefig(str(galaxy_name) + "_distances_cms.png", dpi=500)
